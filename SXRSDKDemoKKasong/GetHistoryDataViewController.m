...
-(void)viewWillAppear:(BOOL)animated{
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onReadData:) name:notify_key_kkasong_did_read_history_data object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(onReadFinish:) name:notify_key_kkasong_read_history_data_finish object:nil];
...
-(void)onReadData:(NSNotification*)notify{
    ...
    for (NSDictionary* datainfo in datalist) {
        int mode = [[datainfo objectForKey:KKASONG_NOTIFY_PARAM_HISTORY_DATATYPE] intValue];
        int step = [[datainfo objectForKey:KKASONG_NOTIFY_PARAM_STEP] intValue];
        double timestamp = [[datainfo objectForKey:KKASONG_NOTIFY_PARAM_TIMESTAMP] doubleValue];
        ...
        int hour = [[datainfo objectForKey:KKASONG_NOTIFY_PARAM_HOUR] intValue];
        int minute = [[datainfo objectForKey:KKASONG_NOTIFY_PARAM_MINUTE] intValue];
        NSString* str = [NSString stringWithFormat:@"[%d-%.2d-%.2d %.2d:%2d:00(%.0f)\nmode->[%d],step->[%d]",year,month,day,hour,minute,timestamp,mode,step];
